	...
	public function getQuote($address) {
		...
		if ($status) {
			...
			if ($this->quote_data) {
                ...
                'title'      => $this->language->get('text_title').$this->s,
	...
	private function setQuoteData($caixa){
		...
		foreach ($servicos as $servico) {
			...
			$valor_frete_sem_adicionais = $servico['Valor'] - $servico['ValorAvisoRecebimento'] - $servico['ValorMaoPropria'] - $servico['ValorValorDeclarado'];
				...
				if(((int)$servico['Erro'] == 0 || (int)$servico['Erro'] == 10) && $valor_frete_sem_adicionais > 0) {
                if((int)$servico['Erro'] == 10) {
                $this->s = "<br><div class='attention'>Área com entrega temporariamente sujeita a prazo diferenciado.</div>";
                } else {
                $this->s = '';
                }
